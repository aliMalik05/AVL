...
void LSLL<T>::sortByReference()
{
	...
	{
		prev2 = prev1->next;
		Node<T>* smallest = prev1;
		Node<T>* exchange = prev1;
		while (prev2 != nullptr)
		{
			if (prev2->info < smallest->info)
			{
				temp3 = temp2;
				smallest = prev2;
			}
		...
		if (head->next == smallest && exchange == head)
		{
		...
		else if (exchange == head)
		{
			Node<T>* tempforpointers = head->next;
			head->next = smallest->next;
			smallest->next = tempforpointers;
			tempforpointers = head;
			head = smallest;
			temp3->next = tempforpointers;
			prev1 = head;
		}
		else if (smallest == exchange->next && exchange->info > smallest->info)
		{
			exchange->next = smallest->next;
			smallest->next = temp->next;
			temp->next = smallest;
			prev1 = temp->next;
		}
		else if (exchange->info > smallest->info)
		{
			Node<T>* tempforpointers = exchange->next;
			exchange->next = smallest->next;
			smallest->next = tempforpointers;
			tempforpointers = exchange;
			temp->next = smallest;
			temp3->next = exchange;
			prev1 = temp->next;
		}
